...
`);
      ...
      const testEntry =
        '@article{test, title = "macro  text " # "blah blah   ding dong "}';
      const testResult = parser.parseString(testEntry, "string input");
      expect(testResult[0].fields.title).toBe(
...
`);
  ...
  describe("Preamble Entry", () => {
    ...
    test("should parse preamble entry correctly", () => {
      ...
      expect(result[0].fields.content).toBe(
...
`);
  ...
  describe("Simple File", () => {
    ...
    test("should parse file with multiple entry types correctly", () => {
      ...
      const bookEntry = result.find(
        (entry) => entry.metatype === BtMetatype.REGULAR
      );
...
`);
  ...
  describe("Biber Example", () => {
    const biberPath = path.join(__dirname, "data", "biber_example.bib");

    test("should parse biber example correctly", () => {
      const content = fs.readFileSync(biberPath, "utf8");
      const parser = new Parser();
      const result = parser.parseString(content);
